# LE HID Keyboard Sample application

## Overview

The Keyboard application is a single chip SoC implementation. It provides a turnkey solution using on-chip keyscan HW component. It operates in Bluetooth LE Mode using HID over GATT Profile (HOGP).

During initialization the app registers with Bluetooth LE Stack, the AIROC™ HID Device Library, and keyscan HW to receive various notifications including bonding complete, connection status changes, peer GATT requests/commands and interrupts for key pressed/released.
...
- Handling of Use AIROC&#8482; HCI UART Control Protocol
...
4. Route to the "HID Device" tab and click on the "Enter Pairing Mode" button to make the HID device discoverable.
![](./images/figure1.png)
5. Pair and connect with the device using an HID Host like a PC, Tablet or another host device.
6. Click on the key buttons to send the key reports.
![](./images/figure2.png)
7. To send keyboard reports, use Interrupt channel, Input report. Enter the contents of the report and click on the Send button, to send the report.For example to send button down event when key '1' is pushed, report should be 01 00 00 1e 00 00 00 00 00.  All buttons up 01 00 00 00 00 00 00 00 00.
